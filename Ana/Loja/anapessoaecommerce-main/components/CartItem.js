...
const productFormated = (item.price * item.quantity)
const itemPrice = productFormated.toLocaleString('pt', {style: 'currency', currency: 'EUR'})
    return (
        <tr>
            ...
            <td style={{minWidth: '200px'}} className="w-50 align-middle" >
                ...
                <h6 className="text-danger">{itemPrice}</h6>
                {/* {
                    item.inStock > 0
                    ? <p className="mb-1 text-danger">Em Stock: {item.inStock}</p>
                    : <p className="mb-1 text-danger">Sem Stock</p>
                } */}
            </td>
