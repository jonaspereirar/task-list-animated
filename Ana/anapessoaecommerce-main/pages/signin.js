      ...
      <div>
        ...
        <form className="mx-auto my-4" style={{maxWidth: '500px'}} onSubmit={handleSubmit}>
          <div className="form-group">
            ...
            name="email" value={email} onChange={handleChangeInput} placeholder="Seu email" />
            <small id="emailHelp" className="form-text text-muted">Nunca compartilharemos seu e-mail</small>
          </div>
          <div className="form-group">
            <label htmlFor="exampleInputPassword1">Senha</label>
            <input type="password" className="form-control" id="exampleInputPassword1"
            name="password" value={password} onChange={handleChangeInput} placeholder="Sua senha" />
          </div>
          
          <button type="submit" className="btn btn-dark w-100">Entrar</button>

          <p className="my-2">
          Você não possui uma conta? <Link href="/register"><a style={{color: 'crimson'}}>Registe-se</a></Link>
          </p>
        </form>
      </div>
    )
