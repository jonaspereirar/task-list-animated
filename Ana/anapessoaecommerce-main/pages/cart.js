...
const Cart = () => {
  ...
  const productFormated = (total)
  const formatedTotal = productFormated.toLocaleString('pt', {style: 'currency', currency: 'EUR'})
  ...
  const handlePayment = async () => {
    if(!address || !mobile)
    return dispatch({ type: 'NOTIFY', payload: {error: 'Por favor, adicione seu endereço e contacto.'}})
      ...
      <div className="row mx-auto">
        ...
        <div className="col-md-4 my-3 text-right text-uppercase text-secondary">
            <form>
              ...
              <input placeholder="digite seu endereço completo" type="text" name="address" id="address"
              className="form-control mb-2" value={address}
              onChange={e => setAddress(e.target.value)} />

              <label htmlFor="mobile">Contacto</label>
              <input placeholder="digite seu contacto" type="text" name="mobile" id="mobile"
              className="form-control mb-2" value={mobile}
              onChange={e => setMobile(e.target.value)} />
            </form>

            <h3>Total: <span className="text-danger">{formatedTotal}</span></h3>

            
            <Link href={auth.user ? '#!' : '/signin'}>
              <a className="btn btn-dark my-2" onClick={handlePayment}>Efetuar pagamento</a>
            </Link>
            <Link href={auth.user ? '/' : '/signin'}>
